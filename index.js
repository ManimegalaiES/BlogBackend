...
app.get('/total-subscribers', async (req, res) => {
    try {
        ...
        console.log("Fetched Blogs:", blogs); // Debugging output

        let totalSubscribers = 0;
        
        blogs.forEach(blog => {
            if (blog.subscribers && Array.isArray(blog.subscribers)) {
                totalSubscribers += blog.subscribers.length;
            }
        });

        console.log("Total Subscribers Calculated:", totalSubscribers); // Debugging output

        res.status(200).json({ totalSubscribers });
    } catch (err) {
        console.error("Error fetching total subscribers:", err);
        res.status(500).json({ message: 'Failed to fetch subscribers count' });
    }
});
...
const transporter1 = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: 'manimegalaielangovan397@gmail.com',
      pass: 'hlam saeq lfux osai',
    },
  });
  ...
  const sendNotificationEmail = (email, transporter) => {
    const mailOptions = {
      from:"manimegalaielangovan397@gmail.com",  // Sender email
